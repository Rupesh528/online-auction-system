...
export const updateAuction = async (req, res) => {
    try {
        ...
        const updatedAuction = await AuctionItem.findByIdAndUpdate(req.params.id, { $set: updates }, { new: true, runValidators: true }).populate("createdBy", "name email");
...
export const placeBid = async (req, res) => {
    try {
        ...
        const updated = await AuctionItem.findOneAndUpdate({ _id: req.params.id, currentBid: { $lt: amount } }, {
...
export const getGlobalStats = async (_req, res) => {
    try {
        ...
        const totalUsers = await AuctionItem.distinct("createdBy").then((users) => users.length);
...
export const getUserStats = async (req, res) => {
    try {
        ...
        const activeAuctions = userAuctions.filter((auction) => new Date(auction.endTime) > new Date()).length;
        const endedAuctions = totalAuctions - activeAuctions;
        // Calculate total value of auctions created (sum of current bids)
        const totalValueCreated = userAuctions.reduce((sum, auction) => sum + auction.currentBid, 0);
        // Get auctions where user has placed bids (not their own)
        const auctionsWithUserBids = await AuctionItem.find({
            "bids.user": userId,
            createdBy: { $ne: userId },
        });
        const totalBidsPlaced = auctionsWithUserBids.reduce((sum, auction) => sum +
            auction.bids.filter((bid) => bid.user?.toString() === userId).length, 0);
        ...
        const totalValueWon = wonAuctions.reduce((sum, auction) => sum + auction.currentBid, 0);
        const stats = {
            totalAuctions,
            activeAuctions,
            endedAuctions,
